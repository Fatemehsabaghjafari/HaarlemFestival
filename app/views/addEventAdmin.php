...
<form id="addEventForm" onsubmit="return validateForm()">
    ...
    <div class="form-group">
        ...
        <input type="number" class="form-control" id="ticketsAvailable" name="ticketsAvailable" required min="1">
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
        <label for="allDaysAccessPrice">All Days Access Price:</label>
        <input type="number" class="form-control" id="allDaysAccessPrice" name="allDaysAccessPrice" required min="0.01" step="0.01">
    </div>
    <div class="form-group">
        <label for="oneDayAccessPrice">One Day Access Price:</label>
        <input type="number" class="form-control" id="oneDayAccessPrice" name="oneDayAccessPrice" required min="0.01" step="0.01">
    </div>
    <div class="form-group">
...
<script>
    document.getElementById('dateTime').addEventListener('input', function() {
        ...
        if (dateTimeValue) {
            var date = dateTimeValue.split('T')[0];
            var time = dateTimeValue.split('T')[1];
    ...
    function validateForm() {
        ...
        for (let input of inputs) {
            if (input.value < input.min) {
                alert(input.name + ' should be at least ' + input.min);
                return false;
            }
        }
